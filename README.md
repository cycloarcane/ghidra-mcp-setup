...
stripped binary as GNU coreutils `whoami` — from the `entry` → `__libc_start_main`
flow and the `geteuid` → `getpwuid` → `puts` call pattern. No symbols, no
cloud. Click to see the full reasoning trace.*
